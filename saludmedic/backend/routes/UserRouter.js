...
router.post("/", (req, res) => {
  ...
  bcrypt.hash(password, saltRound, (err, hash) => {
    ...
    db.query(
      "INSERT INTO prueba (codigo , dni  ,nombre , apellido , password , rol ) VALUES (? , ? , ? , ? , ?  , 'asegurado') ;",
      ...
      (err, result) => {
        try {
          ...
          if (result) {
            db.query(
              "insert INTO familiar (nombre , codigo , dni )  select nombre , codigo , dni from SaludMedic.prueba  where codigo = '" +
                codigo +
                "';"
            );
            res.send(result);
