...
app.get("/api/sendCamas", async (req, res) => {
  ...
  try {
    return await db.query(
      " SELECT DISTINCT f.idFamiliar ,f.nombre , c.estado ,c.especialidad FROM familiar as f  join citas as c on c.nombre = f.nombre where codigo = '" +
        codigo +
        "' and c.fecha = curdate() ; ",
      (err, result) => {
        res.send(result);
...
app.get("/especialista", async (req, res) => {
  return await db.query(
    " select p.nombre  , count(*) as limite from citas as c join prueba as p on c.especialidad = p.especialidad   where  fecha= curdate() group by p.nombre ",
    (err, result) => {
      try {
        res.send(result);
      } catch (err) {}
...
app.get("/doctor/pacientes", async (req, res) => {
  ...
  try {
    return await db.query(
      "select * from citas where  especialidad = '" +
        especialidad +
        "' and estado = 1 ",
      (err, result) => {
        res.send(result);
...
app.post("/doctor/registroPaciente", async (req, res) => {
  ...
  try {
    return await db.query(
      'update citas set estado = "0" where id = "' + id + '";',
      (err, result) => {
...
app.get("/doctor/tabla", async (req, res) => {
  ...
  try {
    return await db.query(
      ' SELECT id ,nombre  , estado , date_format(fecha, "%d-%m-%Y") as fecha , hora  FROM citas   where  fecha= curdate()  and especialidad = "' +
        especialidad +
        '" ;',
      (err, result) => {
